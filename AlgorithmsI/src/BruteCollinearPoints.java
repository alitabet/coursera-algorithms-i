...
public class BruteCollinearPoints {
    ...
    private ArrayList<LineSegment> lineSegments; // array of line segments with 4 collinear points
    ...
    public BruteCollinearPoints(Point[] p) {
        // Brute for ce approach, search all
        // combinations of 4 points and store
        // those with 4 collinear points
                ...
                for (int k = j + 1; k < points.length; k++) {
                    ...
                    for (int w = k + 1; w < points.length; w++) {
                        if (points[i].slopeTo(points[k]) == points[i].slopeTo(points[w])) {
                            ...
                            lineSegments.add(new LineSegment(temp[0], temp[temp.length - 1]));
                        }
                    }
                }
            }
    ...
    private void checkPoints(int i, int j) {
        if (points[i].compareTo(points[j]) == 0) {
            StdOut.println("Points at " + i + " and " + j + " are equal");
